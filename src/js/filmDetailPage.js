...
const handleFilmDetailPage = receivedFilm => {
  ...
  const clearFilmWatchedFromLocalStorage = receivedFilm => {
    let foundWatchedFilm = JSON.parse(
      localStorage.getItem('filmsWatched'),
    ).find(obj => obj.id === receivedFilm.id);
    if (!foundWatchedFilm) {
      return;
    }
    arrayFromLocalStorage = JSON.parse(localStorage.getItem('filmsWatched'));
    let ObjectToClear = arrayFromLocalStorage.find(
      obj => obj.id === receivedFilm.id,
    );
    arrayFromLocalStorage.splice(
      arrayFromLocalStorage.indexOf(ObjectToClear),
      1,
    );
    localStorage.setItem('filmsWatched', JSON.stringify(arrayFromLocalStorage));
  };
  ...
  const clearFilmQueueFromLocalStorage = receivedFilm => {
    let foundQueueFilm = JSON.parse(localStorage.getItem('filmsQueue')).find(
      obj => obj.id === receivedFilm.id,
    );
    if (!foundQueueFilm) {
      return;
    ...
    arrayFromLocalStorage = JSON.parse(localStorage.getItem('filmsQueue'));
    arrayFromLocalStorage.splice(
      arrayFromLocalStorage.indexOf(foundQueueFilm),
      1,
    );
    localStorage.setItem('filmsQueue', JSON.stringify(arrayFromLocalStorage));
  };
  ...
  const checkTheStatusOfWatchedFilm = receivedFilm => {
    let foundFilm = JSON.parse(localStorage.getItem('filmsWatched')).find(
      obj => obj.id === receivedFilm.id,
    );
    if (!foundFilm) {
  ...
  const checkTheStatusOfQueueFilm = receivedFilm => {
    let foundFilm = JSON.parse(localStorage.getItem('filmsQueue')).find(
      obj => obj.id === receivedFilm.id,
    );
    if (!foundFilm) {
  ...
  refs.ToWatchedButtonRef.addEventListener('click', event =>
    handleToWatched(event),
  );
  refs.ToQueueButtonRef.addEventListener('click', event =>
    handleToQueue(event),
  );
};
